...
def test_orm_tables_and_columns_match_frozen_physical_model() -> None:
    expected = _physical_tables(CANONICAL_SQL.read_text())
    actual = {
...
def test_traceability_covers_every_physical_column() -> None:
    ...
    assert manifest["schema_sha256"] == hashlib.sha256(CANONICAL_SQL.read_bytes()).hexdigest()
