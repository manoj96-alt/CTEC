...
from app.domain.shared.value_objects import BusinessName, CanonicalName, Identifier


@dataclass(frozen=True, slots=True)
class Enterprise:
    enterprise_id: Identifier
    enterprise_name: CanonicalName
    legal_name: BusinessName | None
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.enterprise_id, Identifier):
            raise ValidationException("Enterprise ID must be an Identifier")
        if not isinstance(self.enterprise_name, CanonicalName):
            raise ValidationException("Enterprise Name must be a Canonical Name")
        if self.legal_name is not None and not isinstance(self.legal_name, BusinessName):
            raise ValidationException("Legal Name must be a Business Name")
        for field_name, identifier_value in (
            ("Enterprise Type", self.enterprise_type_id),
        ...
        if len(self.enterprise_name.value) > 200:
            raise ValidationException("Enterprise Name must not exceed 200 characters")
        if self.legal_name is not None and len(self.legal_name.value) > 300:
            raise ValidationException("Legal Name must not exceed 300 characters")
