...
`supplier-risk:read` (a different resource — see §5). This name is a
recommendation; the literal string is not frozen until Product Owner
authorization.
...
authority, the normal demo persona SHOULD be granted
...
`ontology-copilot:ask` but withhold `entity-resolution:decide`. Final
grant is a Product Owner decision at authorization time, consistent with how
PAD-002 §11 was decided.
