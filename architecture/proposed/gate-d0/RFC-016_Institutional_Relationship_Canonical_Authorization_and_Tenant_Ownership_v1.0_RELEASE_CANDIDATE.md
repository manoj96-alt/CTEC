...
Version: 1.0 DRAFT
Status: RELEASE CANDIDATE — AUTHORIZED, PENDING REGISTRY PUBLICATION
Current: NO
...
This draft resolves the "Track 1" architecture blocker identified during Priority 6
("Ontology Copilot / Ask CTEC") discovery: whether `institutional_relationships`
— the ECOM Physical Data Model's existing instance-level relationship-edge table
between two `enterprise_entities` — may be exposed through a new read-only
capability, and if so, on what governed footing.
...
This RFC would authorize exactly two changes, both to be delivered in a new
physical model release (not yet generated — see §5):
...
```sql
-- Column addition (second column, immediately after the primary key,
-- matching the placement RFC-015 used on enterprise_entities/source_objects/
-- source_systems):
ALTER TABLE institutional_relationships ADD COLUMN tenant_id VARCHAR(200);
-- (physical model regeneration expresses this as an in-place column
-- addition in CREATE TABLE, per tools/generate_physical_model_release.py's
-- existing pattern; NOT NULL is enforced once backfilled)

-- Existing plain FKs become tenant-qualified, replacing the current
-- definitions in place (same constraint names, matching how RFC-015 replaced
-- fk_source_objects_source_system_id in place rather than adding new
-- differently-named constraints):
ALTER TABLE institutional_relationships DROP CONSTRAINT fk_institutional_relationships_from_entity_id;
...
ALTER TABLE institutional_relationships DROP CONSTRAINT fk_institutional_relationships_to_entity_id;
...
-- Tenant-scoped uniqueness and index, matching the exact pattern used for
-- enterprise_entities/source_objects/source_systems:
...
## 3. Governed artifacts — NOT YET GENERATED, pending §6 authorization

If and when this RFC is authorized, the following artifacts would be
generated, mirroring RFC-015's exact governed-artifacts list and process
(new physical model version file via a dedicated generator script following
`tools/generate_physical_model_release.py`'s pattern; `EAD-001` extended with
a `tenant_id` row for `institutional_relationships`, generated by
`tools/generate_ead_release.py`; `PERSISTENCE-TRACEABILITY` regenerated by
`tools/build_persistence_traceability.py`, which fails closed if the EAD row
is missing; `architecture/INDEX.md` and a new `DEPENDENCY-MATRIX` updated in
the same commit; a new checksummed `RELEASE-MANIFEST` with its sha256 pinned
into `architecture/INDEX.md`'s checksum table; and the corresponding Alembic
migration, landed in the same authorized commit per the RFC-015 precedent of
bundling architecture authorization with its implementing migration). None of
this has been generated yet. This RFC is deliberately submitted before any of
it exists, so the schema decision can be reviewed on its own terms first.

## 4. Non-claims

This RFC does not authorize, and no Gate D0 artifact implements, any Priority 6
...
The Alembic migration and physical-model regeneration described in §3 are
intentionally withheld from this draft. RFC-015's own precedent bundles RFC
authorization and its implementing migration into one commit. The migration is
scoped to land in the same commit as the §3 artifact regeneration, in a
subsequent implementation gate — not in this governance-only gate.

### 5a. Pre-existing row safety (binding requirement on the eventual migration)

The migration must not assume `institutional_relationships` is empty in every
deployed environment merely because no code path in the current application
...
This requirement governs the migration's required behavior; it does not
itself alter §3's deferral of writing that migration now.
...
`institutional_relationships`) and the stated scope in §4 (Non-claims) are
approved, together with the binding pre-existing-row-safety requirement in
§5a. This authorization covers the RFC's decisions and scope. It does not by
itself constitute registry publication: per the Architecture Glossary
(`architecture/released/v1.2/ARCHITECTURE-GLOSSARY-v1.2_FROZEN.md`, line 17),
becoming `FROZEN`/`AUTHORITATIVE`/`Current: YES` additionally requires the §3
artifact regeneration, an atomic `architecture/INDEX.md` registry update, and
Release Manifest regeneration — all still pending, to land together with the
§5 migration in a subsequent implementation gate.
