...
This draft resolves the "Track 2" architecture blocker identified during
Priority 6 ("Ontology Copilot / Ask CTEC") discovery: whether a synchronous,
read-only, deterministic capability may be implemented as ordinary
Product-Layer logic, or whether it must be admitted as a PAC-002 "Question"
through PAD-001's asynchronous Execution-Identifier/poll pattern.
...
governance issues a clarification."* This is submitted as that clarification,
not as a new RFC or CDD — it defines no new canonical entity, attribute,
relationship, Protocol Version, or business semantics, and changes no existing
one. It only draws a boundary that PAD-001's own text leaves implicit.
...
PAD-001's Cognitive Engine boundary in the first place.

## 4. Traceability

PAD-001 v1.5; CIM-001 v1.1 (scope: supplier-risk / single-source-versus-dual-source
vertical slice only); RFC-010 v1.0 §3, §10; RFC-015 v1.0; EAH-001 v1.5;
Architecture Glossary v1.2 (`architecture/released/v1.2/ARCHITECTURE-GLOSSARY-v1.2_FROZEN.md`,
...
definition in §2, and the explicit non-extension to Gate C noted in §1, are
approved. Per the Architecture Glossary (line 17), a clarification additionally
requires "a versioned clarification release, ... an atomic Architecture
Registry update, and regeneration of the applicable Architecture Release
Manifest" before it is `FROZEN`/`Current: YES` — that registry publication
step is still pending and is not performed by this authorization alone.
