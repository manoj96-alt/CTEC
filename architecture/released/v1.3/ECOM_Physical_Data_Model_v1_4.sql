...
-- CDD-012 bounded non-canonical runtime persistence structures.
CREATE TABLE runtime_executions (
    execution_id UUID PRIMARY KEY,
    logical_execution_id UUID NOT NULL,
    tenant_id VARCHAR(200) NOT NULL,
    protocol_version VARCHAR(32) NOT NULL,
    integration_contract_version VARCHAR(32) NOT NULL,
    request_id UUID NOT NULL,
    correlation_id UUID NOT NULL,
    session_id UUID NOT NULL,
    request_classification VARCHAR(100) NOT NULL,
    payload_fingerprint BYTEA NOT NULL,
    control_fingerprint BYTEA NOT NULL,
    ...
    UNIQUE (tenant_id, protocol_version, request_id)
);
...
CREATE TABLE runtime_stages (
    ...
    execution_id UUID NOT NULL REFERENCES runtime_executions(execution_id),
...
CREATE TABLE runtime_handoffs (
    ...
    execution_id UUID NOT NULL REFERENCES runtime_executions(execution_id),
    source_stage VARCHAR(16),
    target_stage VARCHAR(16),
...
CREATE TABLE runtime_artifact_references (
    ...
    execution_id UUID NOT NULL REFERENCES runtime_executions(execution_id),
    stage_id UUID REFERENCES runtime_stages(stage_id),
    artifact_role VARCHAR(40) NOT NULL,
    artifact_id UUID NOT NULL,
    source_capability VARCHAR(16) NOT NULL,
    created_at TIMESTAMPTZ NOT NULL,
...
CREATE TABLE runtime_results (
    ...
    execution_id UUID NOT NULL UNIQUE REFERENCES runtime_executions(execution_id),
    terminal_capability VARCHAR(16),
...
CREATE TABLE runtime_recovery_attempts (
    ...
    checkpoint_stage_id UUID REFERENCES runtime_stages(stage_id),
    tenant_id VARCHAR(200) NOT NULL,
    replay_principal_id VARCHAR(200) NOT NULL,
    original_authorization_reference VARCHAR(200) NOT NULL,
    replay_authorization_reference VARCHAR(200) NOT NULL,
    replay_reason VARCHAR(1000) NOT NULL,
    correlation_id UUID NOT NULL,
...
ALTER TABLE institutional_relationships ADD CONSTRAINT fk_institutional_relationships_from_entity_id FOREIGN KEY (from_entity_id) REFERENCES enterprise_entities(enterprise_entity_id);
ALTER TABLE institutional_relationships ADD CONSTRAINT fk_institutional_relationships_to_entity_id FOREIGN KEY (to_entity_id) REFERENCES enterprise_entities(enterprise_entity_id);
...
ALTER TABLE source_objects ADD CONSTRAINT fk_source_objects_source_system_id FOREIGN KEY (source_system_id) REFERENCES source_systems(source_system_id);
...
-- ---------- CDD-012 BOUNDED NON-CANONICAL RUNTIME PERSISTENCE ----------
CREATE TABLE runtime_executions (
