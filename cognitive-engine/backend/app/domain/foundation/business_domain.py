...
class BusinessDomain:
    ...
    def __post_init__(self) -> None:
        for field_name, identifier_value in (
            ("Business Domain ID", self.business_domain_id),
        ...
        if not isinstance(self.domain_name, CanonicalName):
            raise ValidationException("Domain Name must be a Canonical Name")
        ...
        if len(self.domain_name.value) > 150:
            raise ValidationException("Domain Name must not exceed 150 characters")
