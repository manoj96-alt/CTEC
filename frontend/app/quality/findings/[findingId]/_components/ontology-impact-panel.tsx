...
export function OntologyImpactPanel({ impact }: { impact: OntologyImpactResponse }) {
  if (impact.outcome === "IMPACT_UNKNOWN") {
    return (
      <div>
        <h3>Ontology Impact</h3>
        <p role="status">
          Ontology impact cannot currently be determined from available governed provenance.
        </p>
      </div>
    );
  }
  ...
  if (impact.direct_entity_id) {
    nodes.push({
      ...
      position: { x: 0, y: 60 },
      data: { label: "Finding" },
      style: { border: "1px solid var(--line)", borderRadius: "0.5rem", padding: "0.5rem" },
    });
    nodes.push({
  ...
  propagatedNodes.forEach((segment, index) => {
    ...
    nodes.push({
      ...
      style: { border: "1px dashed var(--line)", borderRadius: "0.5rem", padding: "0.5rem" },
    });
    const previousId =
      index === 0 ? impact.direct_entity_id ?? "finding" : `path-${propagatedNodes[index - 1].relationship_instance_id}-${propagatedNodes[index - 1].path_ordinal}`;
    edges.push({
  ...
  return (
    <div>
      <h3>Ontology Impact</h3>
      <div style={{ height: "16rem", border: "1px solid var(--line)", borderRadius: "0.5rem" }}>
      ...
      <details style={{ marginTop: "0.75rem" }}>
        ...
        <ul>
          {impact.direct_entity_id ? (
            <li>
              Direct impact: {impact.direct_entity_type ?? "Entity"} ({impact.direct_entity_id})
            </li>
          ) : null}
          {propagatedNodes.map((segment) => (
            <li key={`${segment.relationship_instance_id}-${segment.path_ordinal}`}>
              Propagated step {segment.path_ordinal}: relationship {segment.relationship_instance_id} (
              {segment.direction})
            </li>
          ))}
        </ul>
