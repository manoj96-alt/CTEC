      ...
      candidates: [
        {
          ...
          narrative:
            "Gate F governed four-condition mitigation policy (CDD-015 §11).",
          confidence: "High",
          evidence: [
            {
...
test("renders the scenario picker with all three deterministic scenarios", () => {
  ...
  expect(
    screen.getByRole("button", { name: /High-risk supplier/ }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("button", { name: /Missing governed evidence/ }),
  ).toBeInTheDocument();
  expect(
    screen.getByRole("button", { name: /Below the materiality threshold/ }),
  ).toBeInTheDocument();
});
...
test("selecting a scenario calls evaluate with only the supplier entity id", async () => {
  ...
  await waitFor(() =>
    expect(evaluateMock).toHaveBeenCalledWith(
      "5d98f421-ef60-5463-9c4e-f81b8bc63da1",
    ),
  );
...
test("renders RECOMMENDED with structured reasons, narrative, and confidence", async () => {
  ...
  expect(
    screen.getByText(/High/, { selector: "p.standing" }),
  ).toBeInTheDocument();
});
...
test("renders REJECTED distinctly from UNKNOWN", async () => {
  ...
  expect(
    screen.queryByText("Insufficient governed evidence"),
  ).not.toBeInTheDocument();
});
...
test("renders evidence with source, predicate, value, and timestamp", async () => {
  ...
  expect(
    screen.getByText("severity", { selector: "strong" }),
  ).toBeInTheDocument();
...
test("renders a 401 state and offers sign-in", async () => {
  evaluateMock.mockRejectedValue(
    new SupplyChainImpactApiError("AUTH_TOKEN_MISSING", 401),
  );
  render(<SupplyChainImpactPage />);
  clickScenario(/High-risk supplier/);
  await waitFor(() =>
...
test("renders a 403 state distinct from 401 (read-only persona attempting evaluation)", async () => {
  ...
  await waitFor(() =>
    expect(
      screen.getByText("Additional authority required"),
    ).toBeInTheDocument(),
  );
...
test("renders a backend-unavailable state for non-auth API failures", async () => {
  evaluateMock.mockRejectedValue(
    new SupplyChainImpactApiError("HTTP_500", 500),
  );
  render(<SupplyChainImpactPage />);
  clickScenario(/High-risk supplier/);
  await waitFor(() =>
    expect(screen.getByText("Gate F is unavailable")).toBeInTheDocument(),
  );
});
...
test("no business conclusion is computed client-side: only backend-returned outcome/reason/narrative/confidence are ever rendered", async () => {
  ...
  expect(
    screen.getAllByText(recommendedResponse.materials[0].candidates[0].reason!)
      .length,
  ).toBeGreaterThan(0);
  expect(
