...
const EMPTY_BUSINESS_IMPACT = {
  outcome: "NO_KNOWN_BUSINESS_IMPACT",
  dependencies: [],
};
const EMPTY_RELIANCE = {
...
function mockAll(overrides: {
  ...
  findingDetailMock.mockResolvedValue({
    ...BASE_FINDING,
    ...overrides.finding,
  });
  evidenceMock.mockResolvedValue(
    overrides.evidence ?? { participants: [], candidate: null },
  );
  ontologyImpactMock.mockResolvedValue(overrides.impact ?? EMPTY_IMPACT);
  businessImpactMock.mockResolvedValue(
    overrides.businessImpact ?? EMPTY_BUSINESS_IMPACT,
  );
...
describe("OQI Finding Detail — evidence", () => {
  it("N-source agreement: 4 governed peers + 1 missing, candidate labeled not-truth", async () => {
    ...
    expect(
      screen.getByText("4 governed peers observed ABC123"),
    ).toBeInTheDocument();
    expect(
...
describe("OQI Finding Detail — ontology impact", () => {
  ...
  it("provides an accessible non-graph path representation", async () => {
    ...
    const details = screen
      .getByText("Accessible path detail")
      .closest("details");
    expect(details).not.toBeNull();
    expect(
      within(details as HTMLElement).getByText(/Direct impact: MATERIAL/),
    ).toBeInTheDocument();
    expect(
      within(details as HTMLElement).getByText(/Propagated step 1/),
    ).toBeInTheDocument();
  });
});
...
describe("OQI Finding Detail — business impact", () => {
  it("BUSINESS_IMPACT_UNKNOWN renders explicit unknown, never zero/blank", async () => {
    mockAll({
      businessImpact: { outcome: "BUSINESS_IMPACT_UNKNOWN", dependencies: [] },
    });
  ...
  it("contextual criticality: same subject, two dependencies, both preserved distinctly", async () => {
    mockAll({
      businessImpact: {
        ...
        dependencies: [
          {
            business_process_name: "Production Planning",
            criticality: "CRITICAL",
            business_dependency_version: 1,
          },
          {
            business_process_name: "Management Reporting",
            criticality: "MEDIUM",
            business_dependency_version: 1,
          },
        ],
...
describe("OQI Finding Detail — agent investigation", () => {
  it("specialist disagreement is preserved, never voted into a fabricated consensus", async () => {
        ...
        specialists: [
          {
            ...
            assessment_text: "Favors ABC123",
            referenced_candidate_id: "c1",
          },
          {
            role_id: "impact-analyst",
            result_state: "SUCCEEDED",
            assessment_text: "Favors XYZ999",
            referenced_candidate_id: "c2",
          },
        ],
        recommendation: {
          recommendation_type: "RECOMMEND_CANDIDATE",
          candidate_id: "c1",
          rationale: "Four peers agree",
    ...
    expect(
      screen.getByText("Based on specialist assessments"),
    ).toBeInTheDocument();
    ...
    mockAll({
      agent: {
        ...
        recommendation: {
          recommendation_type: "RECOMMEND_CANDIDATE",
          candidate_id: "c1",
          rationale: "synthesizer reasoning",
          basis: "SYNTHESIZER_ONLY",
        },
      },
    });
    await renderTab("Agent Investigation");
        ...
        specialists: [
          {
            role_id: "evidence-analyst",
            result_state: "SUCCEEDED",
            assessment_text: "<img src=x onerror=alert(1)>",
  ...
  it("stale authorization is visibly non-actionable", async () => {
    ...
    expect(
      screen.queryByRole("button", {
        name: /apply|execute|use authorization/i,
      }),
